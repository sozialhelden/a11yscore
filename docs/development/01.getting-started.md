...
**4. Start local databases and redis**
```bash
docker-compose up -d
```

**5. Run migrations**
Wait a few moments until the databases are fully up and running, then run the migrations:
```bash
# Local osm-database used for development
npm run db:migrate
# Local osm-database used for tests
...
```
Inspect the [package.json](../../package.json) for all available scripts.
